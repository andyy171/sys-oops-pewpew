# — Các lệnh Linux thường gặp 
## Lệnh cơ bản trên hệ thống
...
![](/01-linux/images/lenhdiff.png)

- Xác định kiểu file: `# file <duong_dan_toi_file>`
## 2. Lệnh nén và giải nén
...
![](/01-linux/images/cautrucphancap.png)
...
chown -v tubui
```
![](/01-linux/images/chgrp.png)



## 5. Làm việc với người dùng và nhóm 
...
### `ip -s link show <tên_card>` - tương đương với lệnh ifconfig
Dùng để kiểm tra các bộ đếm lỗi phần cứng và gói tin bị hủy (errors, dropped) trực tiếp trên giao diện mạng.
### `dmesg | grep -iE 'eth|enp|eno|link'`
Dùng để kiểm tra nhật ký hệ thống nhằm phát hiện các sự cố vật lý như lỏng cáp, mất kết nối (link down) hoặc lỗi trình điều khiển (driver).
### `mtr <địa_chỉ_IP>`
Dùng để chẩn đoán mất gói tin (Loss %) và độ trễ trên toàn bộ lộ trình từ server đến đích, giúp xác định lỗi do server hay do nhà mạng.
