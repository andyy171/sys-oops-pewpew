...
Thực hiện trên **cả 4 node** (`node1.ceph.local`, `node2.ceph.local`, `node3.ceph.local`, `node4.ceph.local`)

```bash
sudo hostnamectl set-hostname node1.ceph.local  # trên node01
sudo hostnamectl set-hostname node2.ceph.local  # trên node02  
sudo hostnamectl set-hostname node3.ceph.local  # trên node03
sudo hostnamectl set-hostname node4.ceph.local  # trên node04

# Cập nhật /etc/hosts
cat << EOF | sudo tee /etc/hosts
127.0.0.1   localhost
192.168.56.101 node1 node1.ceph.local
192.168.56.102 node2 node2.ceph.local
192.168.56.103 node3 node3.ceph.local
192.168.56.104 node4 node4.ceph.local
EOF


cat << EOF | sudo tee /etc/hosts
127.0.0.1   localhost
192.168.56.201 node5 node5.ceph.local
192.168.56.202 node6 node6.ceph.local
192.168.56.203 node7 node7.ceph.local
EOF
# Kiểm tra hostname
hostname -f
...
```bash
# Thêm n hard disk phụ trên mỗi node 
# Disable swapoff
sudo swapoff -a
...
# echo "deb [signed-by=/usr/share/keyrings/ceph-archive-keyring.gpg] https://download.ceph.com/debian-squid noble main" | sudo tee /etc/apt/sources.list.d/ceph.list

# Hiện tại Ceph upstream chưa có bản hỗ trợ cho ubuntu 24.04 nên có thể cài ceph-common do ubuntu build
...
ceph dashboard set-grafana-api-url http://192.168.56.201:3000
...
sudo ceph orch host add node2.ceph.local 192.168.56.102
sudo ceph orch host add node3.ceph.local 192.168.56.103  
sudo ceph orch host add node4.ceph.local 192.168.56.104
...
```bash
sudo ceph orch apply mon --placement "node1.ceph.local,node2.ceph.local,node3.ceph.local"
...
```bash
sudo ceph orch apply mgr --placement "node1.ceph.local,node2.ceph.local,node3.ceph.local"
...
## Deploy RGW instance
ceph orch apply rgw myrealm myzone --placement="node1.ceph.local"
...
ceph logs rgw.myzone.node1.ceph.local
