title: "Hướng dẫn các thao tác tìm kiếm và sửa lỗi"
...
## MON clock skew
- **Clock skew** là tình trạng thời gian hệ thống giữa các node trong cluster bị lệch nhau.  
Trong Ceph, đặc biệt là giữa các **MON**, sự lệch thời gian này có thể gây cảnh báo:
...
- Service đồng bộ thời gian (**chrony / NTP**) không chạy
- Node không kết nối được tới NTP server
- Thay đổi thời gian thủ công (`date -s`)
- VM clock drift
- Network issue làm node mất sync
...
### Xử lý nhanh
```bash
# Restart service đồng bộ thời gian:
systemctl restart chronyd
## Sau đó chờ vài phút để cluster tự đồng bộ lại.
# Nếu cảnh báo vẫn còn thì restart lại mon :
