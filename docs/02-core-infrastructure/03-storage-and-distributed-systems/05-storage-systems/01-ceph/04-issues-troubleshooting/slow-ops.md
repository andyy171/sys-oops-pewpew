...
Các ảnh dưới đây được giữ lại từ `_inbox` như bằng chứng lab nhỏ cho luồng nhận diện slow ops:

![Ceph health OK before slow ops test](./images/slow-ops-health-ok-before.png)

![OSD block device visible from lsblk](./images/slow-ops-osd-block-device.png)

![BlueStore slow operation alert in cluster log](./images/slow-ops-bluestore-alert-log.png)

Khi đọc ảnh/log kiểu này, hãy chú ý ba điểm:

- `ceph -s` ban đầu `HEALTH_OK` giúp chứng minh cluster ở trạng thái sạch trước khi test.
- `lsblk` xác định thiết bị nào đang được dùng làm OSD block device.
- Log `BLUESTORE_SLOW_OP_ALERT` và `waiting for subops` cho thấy OSD đang chờ sub-operation, thường cần kiểm tra disk latency, network giữa OSD, recovery/backfill hoặc RocksDB/BlueStore stall.
