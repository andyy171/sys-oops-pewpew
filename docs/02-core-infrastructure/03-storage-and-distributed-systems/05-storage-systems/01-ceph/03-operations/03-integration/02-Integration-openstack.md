...
Cấu hình này chỉ nên xem là lab shortcut để copy file và chạy lệnh từ xa giữa các node mà không cần sudo lặp lại. Với production, ưu tiên SSH key, bastion/jump host, sudo được audit và tắt password login sau khi hoàn tất.
...
## Validation Sau Khi Reconfigure

Sau khi Kolla render lại config, kiểm tra cả phía OpenStack và phía Ceph:

```bash
source /etc/kolla/admin-openrc.sh
openstack image list
openstack volume type list
openstack volume create --size 1 ceph-smoke-volume
...
Nếu tạo volume hoặc boot instance lỗi, kiểm tra theo thứ tự:

1. File `ceph.conf` trong container có đúng monitor endpoint không.
2. Keyring được copy đúng thư mục service chưa.
3. Ceph caps của `client.glance`, `client.cinder`, `client.nova` có đúng pool không.
4. Pool đã `rbd pool init` chưa.
5. Container Glance/Cinder/Nova đã được reconfigure/restart chưa.
6. Log service có lỗi auth, permission denied hoặc không tìm thấy pool không.

## Related Pages

- [OpenStack Ceph Infrastructure Planning](./01-openstack-ceph-infrastructure-planning.md)
- [Kolla-Ansible All-In-One Lab](../../../../../04-cloud-edge/02-cloud-ecosystem/openstack/02-operations/01-deployment/kolla-ansible-all-in-one-lab.md)
- [Ceph Common Commands](../ceph-common-commands.md)
