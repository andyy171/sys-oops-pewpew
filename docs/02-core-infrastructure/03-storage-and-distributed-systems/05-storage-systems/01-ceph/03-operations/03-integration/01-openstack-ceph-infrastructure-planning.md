...
$$
Dung\ lượng\ yêu\ cầu\ = Dung\ lượng\ Thực\ Tế × 1.3 – 1.5
$$ 
...
Mạng là yếu tố quan trọng nhất trong Ceph, vì nó xử lý cả lưu lượng frontend (từ client như OpenStack VMs truy cập dữ liệu) và backend (replication, recovery, rebalancing giữa các OSD). Nếu mạng bị nghẽn, toàn bộ cluster có thể bị chậm hoặc treo I/O, đặc biệt trong các tình huống recovery (khi một OSD hoặc node hỏng, Ceph phải di chuyển dữ liệu khổng lồ).

- Phân loại Network trong Ceph:
    - Public Network (Frontend): Lưu lượng từ client (ví dụ: OpenStack Nova/Glance/Cinder) đến Ceph. Đây là nơi VMs đọc/ghi dữ liệu thực tế.
    - Cluster Network (Backend): Lưu lượng nội bộ giữa các OSD để replicate dữ liệu, tính toán parity (trong EC), và recovery. Nên tách riêng khỏi Public để tránh tranh chấp băng thông.
>Khuyến nghị: Sử dụng ít nhất hai mạng riêng biệt (dual NIC hoặc VLAN với QoS). Nếu không tách, recovery có thể "ăn" hết băng thông, làm VMs bị lag.

- Yếu tố ảnh hưởng đến Network Sizing:
    - Replica/Erasure Coding (R): Với replica 3x, mỗi write từ frontend sẽ tạo ra 3x lưu lượng backend. Với EC (k+m), write penalty cao hơn vì cần đọc dữ liệu cũ để tính parity.
    - Workload Intensity: Workloads nặng (như DB hoặc CI/CD) tạo burst traffic cao. Ví dụ: Một write 4KB từ VM có thể biến thành 12KB backend (với replica 3).
    - Recovery/Backfill: Khi một node chết, Ceph có thể đẩy traffic lên đến 1-10 GB/s per OSD trong giờ đầu. Dự phòng 20-50% headroom cho các tình huống này.
    - Latency Requirements: Ceph nhạy cảm với latency; nên dùng switch low-latency (cut-through) và tránh oversubscription ở spine-leaf topology.
    - Overhead: Bao gồm protocol overhead (TCP/IP ~10-20%), compression/encryption (~5-10%), và management traffic (~5%).

- Công thức tính toán Bandwidth:
Để ước lượng băng thông cần thiết, bắt đầu từ frontend throughput (dựa trên IOPS và block size), sau đó nhân với penalty backend.
Frontend Bandwidth (B_Frontend):$$B_{\text{Frontend}} = IOPS_{\text{Front}} \times \text{Block Size} \times (ReadRatio + WriteRatio)$$
**Ví dụ:** Với 10,000 IOPS frontend, block size trung bình 8KB, tỷ lệ 70/30 read/write:
$B_{\text{Frontend}} = 10,000 \times 8KB \times 1 = \approx 80 MB/s$ (chuyển đổi: 10,000 IOPS × 8,192 bytes / 1,000,000 ≈ 80 MB/s).

Backend Bandwidth (B_Backend/Cluster):$$B_{\text{Cluster}} = B_{\text{Frontend}} \times R \times 1.3 \quad \text{(overhead cho protocol và burst)}$$
- `R`: Replica factor (thường 3) hoặc k/(k+m) cho EC nhưng điều chỉnh cho write penalty (EC write thường ~1.5-2x so với read).
- `1.3`: Hệ số overhead an toàn (có thể điều chỉnh lên 1.5-2.0 cho production lớn để bao gồm recovery).
Ví dụ: Với B_Frontend = 80 MB/s, R=3:
$B_{\text{Cluster}} = 80 \times 3 \times 1.3 = 312 MB/s \approx 2.5 Gbps$ (chuyển đổi MB/s sang Gbps: ×8 /1000).

Total Bandwidth per Node:$$B_{\text{Node}} = \max(B_{\text{Frontend per Node}}, B_{\text{Cluster per Node}}) \times 1.25 \quad \text{(headroom cho N+1 failure)}$$
Phân bổ đều cho số node (ví dụ: 5 nodes → chia traffic đều).


- Khuyến nghị Hardware Network dựa trên OSD Type và Scale:

| OSD Type / Workload               | Minimum Bandwidth (per Node)           | Recommended                                    | Notes                                                                 |
|-----------------------------------|----------------------------------------|-----------------------------------------------|-----------------------------------------------------------------------|
| HDD (Low IOPS)                    | 1-10 Gbps (Public + Cluster)           | 10 Gbps (Public) + 10 Gbps (Cluster)          | Phù hợp archive; recovery chậm nhưng traffic thấp.                    |
| SSD (Medium)                      | 10 Gbps (Public) + 10-25 Gbps (Cluster)| 25 Gbps bonded (e.g., 2x10G)                  | Write amplification cao; cần cho general purpose.                     |
| NVMe (High Perf)                  | 25 Gbps (Public) + 40-100 Gbps (Cluster)| 100 Gbps (e.g., 2x50G) với RDMA/InfiniBand    | Latency <1ms; burst traffic có thể >10 GB/s per node.                 |
| Hyper-Converged (HCI với OpenStack)| +10-25% extra                          | Dual 25Gbps NICs (bonded for HA)              | Tranh chấp với VM traffic; dùng SR-IOV/QoS để ưu tiên.                |

- Ví dụ Thực Tế:
    - Cụm 3 Nodes, mỗi Node 12 SSD OSDs, Workload Mixed (Frontend IOPS: 50,000 total, Block 16KB, Replica 3):
        - B_Frontend total = 50,000 × 16KB ≈ 800 MB/s (chia đều 3 nodes → ~267 MB/s/node ≈ 2.1 Gbps).
        - B_Cluster = 800 × 3 × 1.3 ≈ 3,120 MB/s total (≈1,040 MB/s/node ≈ 8.3 Gbps).
    - Đề xuất: Mỗi node dùng 10 Gbps Public + 25 Gbps Cluster (bonded 2x25G cho HA).
    - Recovery Check: Nếu 1 node chết (12 OSDs down), traffic recovery ≈ 1-2 GB/s per remaining node → cần headroom để tránh saturation.


- Best Practices và Pitfalls:
    - Monitor Traffic: Sử dụng Ceph dashboard hoặc Prometheus để theo dõi utilization; giữ dưới 70% peak để tránh bottleneck.
    - Jumbo Frames: Bật MTU 9000 để giảm overhead packet, tăng throughput ~10-20%.
    - Avoid Oversubscription: Ở switch level, giữ ratio 4:1 (downlink:uplink) cho leaf-spine; tránh 20:1 trừ khi workload nhẹ.
    - Pitfall: Trong HCI (Ceph + OpenStack cùng node), storage traffic có thể "ăn" hết NIC, làm VMs mất kết nối
    - Giải pháp: Dùng OVS-DPDK hoặc SR-IOV để offload.
    - Scale Consideration: Với >10 nodes, xem xét RDMA (RoCE) để giảm CPU overhead từ network processing.
> 10 GbE per 12 OSDs 
