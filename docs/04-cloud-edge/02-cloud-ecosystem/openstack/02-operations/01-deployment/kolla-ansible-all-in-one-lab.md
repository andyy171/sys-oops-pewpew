...
Note này là một runbook lab để dựng OpenStack all-in-one bằng Kolla-Ansible. Đây là môi trường học/lab, không phải blueprint production. Các IP, interface và release trong ví dụ cần được thay theo lab thật.Ví dụ sử dụng Ubuntu 24.04 và OpenStack/Kolla nhánh 2024.2.
...
## KVM / Nested Virtualization Check

Nếu lab Kolla-Ansible chạy bên trong VM, Nova có thể dùng `qemu` khi không có KVM, nhưng performance sẽ thấp. Trước khi debug lỗi spawn VM, kiểm tra host lab có thấy hardware virtualization không:
...
| Kết quả | Ý nghĩa |
|---|---|
| Có `vmx` hoặc `svm` | CPU flag VT-x/AMD-V được expose vào VM/host lab. |
| Có `kvm_intel` hoặc `kvm_amd` | Kernel module vendor-specific đã load. |
| Có `/dev/kvm` | Nova/libvirt có cơ hội dùng KVM acceleration. |
| Không có `/dev/kvm` | Dùng `nova_compute_virt_type: "qemu"` cho lab hoặc bật nested virtualization ở hypervisor bên ngoài. |
