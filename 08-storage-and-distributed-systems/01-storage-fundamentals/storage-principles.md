# Các khái niệm cơ bản về hệ thống lưu trữ
...
- **Throughput:** Tốc độ truyền dữ liệu, đo bằng MB/s hoặc Gb/s. Ví dụ, sequential read trên SSD có thể đạt 500MB/s.
- **IOPS (I/O Operations Per Second):** Số lệnh I/O mỗi giây, quan trọng cho random access.
- **Latency:** Thời gian hoàn thành một I/O, ảnh hưởng trải nghiệm người dùng.

>Một số benchmark chỉ đo throughput mà bỏ qua IOPS ở workload random, dẫn đến đánh giá sai (ví dụ HDD tốt sequential nhưng kém random)

>Trong cloud, network latency có thể làm thay đổi hierarchy.
